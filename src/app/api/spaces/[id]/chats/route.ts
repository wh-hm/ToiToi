...
export async function DELETE(
    ...
    try {
        ...
        const success = await deleteSpaces(auth.user_id, "CHAT"); 
        
        if (!success) return NextResponse.json({ error: MESSAGES.E2004("チャットスペース全削除") }, { status: 500 });
        
        return NextResponse.json({ message: MESSAGES.S1003("チャットスペース全削除") });
    } catch (error) {
        return NextResponse.json({ error: MESSAGES.E2004("チャットスペース全削除") }, { status: 500 });
    }
}